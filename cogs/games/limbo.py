...
class Limbo(commands.Cog):
    ...
    @commands.hybrid_command(name="limbo", aliases=["lm"], description="Predict how high the multiplier climbs before it crashes.")
    @app_commands.describe(bet="Amount to bet (or 'half' / 'all')", target="Target multiplier, e.g. 2.0")
    async def limbo(self, ctx: commands.Context, bet: str, target: float):
        if target < 1.01:
        ...
        msg = await ctx.send(embed=base_embed(f"{E_ROCKET} Launching...", f"Target: {target}x", COLOR_PRIMARY))
        await suspense(msg, f"{E_ROCKET} Launching...", f"Target: {target}x")
        ...
        desc = f"Rocket reached **{result}x** (needed {target}x)\n"
        if won:
